 ...
 * any list yet. The initialization is done by macro TLL_INIT(ptr)
 * 
 * If you declare a list head as a static variable, you may define it as
 * TLL_HEAD(your_list_head) so it will be initialized, and there is no need
 * to call TLL_INIT(&your_list_head) later.
 *
 * Use macros TLL_FOREACH(head, ptr) and TLL_FOREACH_SAFE(head, ptr, temp_ptr)
 ...
 * can be obtained by TLL_ENTRY(ptr, struct_type, node_name) macro.
 ...
 * static TLL_HEAD(host_list_head);
 *
 ...
 * 		TLL_ADD(&host_list_head, &hp->host_list);
 * 	}
 *
 * 	return (hp);
 * }
 ...
 * struct llhead *lp;
 * struct host *hp;
 * TLL_FOREACH(&host_list_head, lp) {
 * 	hp = TLL_ENTRY(lp, struct host, host_list);
 * 	printf("%s\n", hp->host_name);
 * }
 *
 ...
 * struct host *hp;
 * TLL_FOREACH_SAFE(&host_list_head, lp, tmp) {
 * 	hp = TLL_ENTRY(lp, struct host, host_list);
 * 	free(hp->host_name);
 * 	free(hp);
 * }
 ...
 * ----------
 *
 *  TLL_INIT:  initializes the list
 *  TLL_ENTRY: convert list pointer to the pointer to struct that
 *  	contains that list. t - type of a struct, p - ptr to list head,
 *  	n - name of a list node in a struct
 *  TLL_ADD: add an entry to the list, stack implementation
 *  TLL_TAIL: add an entry to the list, queue implementation
 *  TLL_DEL: remove an entry from the list
 *  TLL_FOREACH: loop over all elements in a list. h - head, p - iterator
 *  TLL_FOREACH_SAFE: loop over all elements in a list. h - head,
 *  	p - iterator, tmp - temporary storage
 *  	this macro is safe to use for the list element removal.
 *  TLL_EMPTY: nonzero if the list is empty, 0 otherwise
 *
...
#define	TLL_INIT(N)	((N)->next = (N)->prev = (N))

#define TLL_HEAD(H)	struct llhead H = { &H, &H }

#define	TLL_ENTRY(P,T,N)	((T *) ((char *) (P) - offsetof(T,N)))
...
#define	TLL_EMPTY(N)	((N)->next == (N))

#define	TLL_FOREACH(H,N)	for (N = (H)->next; N != (H); N = (N)->next)

#define TLL_FOREACH_SAFE(H,N,T)						\
