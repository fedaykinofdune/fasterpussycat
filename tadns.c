...
#define DESTROY_QUERY(head,query) DL_DELETE(head,query); if(query) ck_free(query);
...
struct dns {
	...
	struct query	*active;		/* Active queries, MRU order	*/
	struct query  *cached;		/* Cached queries		*/
...
dns_init(void)
{
	...
	if ((dns = (struct dns *) ck_alloc(sizeof(*dns))) == NULL)
		return (NULL);
...
call_user(struct dns *dns, struct query *query, enum dns_error error)
{
	...
	/* Move query to cache */
	if(find_active_query(dns, query->tid)) DL_DELETE(dns->active, query);
  if(!find_cached_query(dns, query->qtype, query->name) && error!=DNS_TIMEOUT) {
    DL_PREPEND(dns->cached, query);
	  dns->num_cached++;
  }
	if (dns->num_cached >= MAX_CACHE_ENTRIES) {
		query = dns->cached->prev;
    if(query) { 
...
dns_poll(struct dns *dns)
{
	...
	DL_FOREACH_SAFE(dns->active, query, tmp) {
...
dns_fini(struct dns *dns)
{
	...
	DL_FOREACH_SAFE(dns->active, query, tmp) {
		DESTROY_QUERY(dns->active, query);
	}
		...
		enum dns_query_type qtype, dns_callback_t callback)
{
	...
	/* Allocate new query */
	if ((query = (struct query *) ck_alloc(sizeof(*query))) == NULL) {
