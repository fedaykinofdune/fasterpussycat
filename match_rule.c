...
struct match_rule *new_404_rule(struct match_rule **list){
...
inline not_head_method(struct http_request *req){
  return !strcmp(req->method, "HEAD");
}

int rule_matches(struct match_rule *rule, struct http_request *req, struct http_response *res){
  if(rule->method!=DETECT_ANY && strcmp(req->method,rule->method)) return 0;
  if(rule->code!=DETECT_ANY && rep && rep->code!=rule->code) return 0;
  if(rule->size!=DETECT_ANY 
    && res
    && GET_HDR((unsigned char *) "content-length", &res->hdr) 
    && atoi(GET_HDR((unsigned char *) "content-length", &res->hdr)!=rule->size)) return 0;
  if(test_flags!=DETECT_ANY && req->test && !(req->test->flags && rule->test_flags)) return 0;
  if(rule->hash!=DETECT_ANY 
    && res
    && not_head_method(req) 
    && res->hash 
    && memcmp(res->hash,rule->hash,MD5_DIGEST_LENGTH)) return 0;
  if(rule->pattern!=DETECT_ANY && !regexec(rule->pattern, serialize_path(req,0,0), 0, NULL, 0)) return 0;
